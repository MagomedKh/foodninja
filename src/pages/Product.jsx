...
export default function Product() {
    ...
    const {addon_products, cartProducts, products} = useSelector( ({products, cart}) => {
      return {
    ...
    const [product] = React.useState(Object.values(products).find(item => item.slug === productSlug[2]));
    ...
    React.useEffect(() => {
        if( product ) {
            ...
            if( product.type === 'variations' && product.attributes ) {
                ...
                dataAttributes[index] = Object.values(product.attributes)[index].options[0];
                });
            ...
            { product !== undefined ? (
                <div className="product-modal">
                    <div className="product-modal--image">
                        <Zoom in={true}><img src={ activeVariant ? activeVariant.img : product.img } alt={product.title}/></Zoom>
                    </div>
                    <div className="product-modal--info no-bg">
                        <h1 className="product-modal--title" style={{marginBottom: 5}}>{product.title}</h1>

                        { product.type === 'variations' ? (
                        <>
                            { (activeVariant && activeVariant.description) ?
                            <div className="product-modal--description" data-product-id={product.id} dangerouslySetInnerHTML={{__html: activeVariant.description}}></div>
                            : <div className="product-modal--description" data-product-id={product.id} dangerouslySetInnerHTML={{__html: product.content.replace(/\n/g,"<br />")}}></div>}

                            <div className="product-modal--attributes">
                            { Object.values(product.attributes).map( (attribute, keyAttr) => (
                        ...
                        <div className="product-modal--buying">
                            ...
                            { activeVariant ? activeVariant.price : product.options._price} &#8381;
                            </div>
                            ...
                            <Button variant="button" className="btn--action btn-buy" onClick={() => handleAddProduct(product)}>Хочу</Button>
                            ) : (
                            <div className="product-modal--quantity">
                                <Button className="btn--default product-decrease" onClick={() => handleDecreaseProduct(product)}>-</Button>
                                <input className="quantity" type="text" readOnly value={cartProducts[product.id].items.length} data-product_id={product.id} />
                                <Button className="btn--default product-add" onClick={() => handleAddProduct(product)}>+</Button>
                            </div>
                            )  }
                        </div>
                        ) : '' }
