...
export default function Cart() {
    ...
    const { config, cart, promocode, promocodeProducts, cartProducts, cartSubTotalPrice, cartTotalPrice, cartCountItems, user, items } = useSelector( ({config, cart, products, user}) => {
        return {
            config: config.data,
            cart: cart,
                ...
                <div>
                    ...
                    <div className="cart-product-list">
                        ...
                        { ( config.CONFIG_cart_info_text !== undefined && config.CONFIG_cart_info_text ) && (
                            <Alert className="minicart--text-info" severity="info" sx={{mb: 2}} ><div dangerouslySetInnerHTML={{__html: config.CONFIG_cart_info_text}}></div></Alert>
                        ) }
                        ...
                        <TransitionGroup>
                            ...
                            { Object.keys(cartProducts).map( (key, index) => 
                                items[key] && items[key].type !== undefined && items[key].type === 'variations' ? cartProducts[key].items.map( (keyVariantProduct, indexVariantProduct) => 
                                    <Collapse key={key} className="collapse-cart-product">
                                        <CartProduct key={indexVariantProduct} productIndex={indexVariantProduct} productCart={cartProducts[key].items[indexVariantProduct]} productCount={1} productTotalPrice={cartProducts[key].items[indexVariantProduct].options._price} />
                                    </Collapse> 
                                ) : <Collapse key={key} className="collapse-cart-product">
                                    <CartProduct key={cartProducts[key].items[0].id} productIndex={0} productCart={cartProducts[key].items[0]} productCount={cartProducts[key].items.length} productTotalPrice={cartProducts[key].totalPrice} />
                                </Collapse>
                            ) }

                            { Object.keys(promocodeProducts).map( (key, index) => items[key] !== undefined &&
                                <PromocodeCartProduct productCart={promocodeProducts[key]} productCount="1" productTotalPrice={promocodeProducts[key].options._price-promocode.amount} />
                            ) }
                            <CartFreeAddons />
                        </TransitionGroup>
                        ...
                        { cart.discount ? (
                            <div className="cart--subtotal-price">
                                ...
                                <div className="promocode">
                                    Промокод <span className="main-color">{promocode.code}</span>: 
                        ...
                        <div className="cart--total-price">
                            <div className="finish-price">Итого: <span className="money">{cartTotalPrice.toLocaleString('ru-RU')} &#8381;</span></div>
                            { config.CONFIG_order_min_price ? cartTotalPrice < config.CONFIG_order_min_price ? <Alert severity="error" sx={{mt: 2}}>Минимальная сумма для заказа - {config.CONFIG_order_min_price} ₽</Alert>
                            : ''
                            : '' }
                        </div>
                        <div className="cart--bottom-buttons">
                            <Button onClick={handleClickBackToMenu} className="btn btn--outline-dark">Вернуться в меню</Button>

                            { config.CONFIG_order_min_price ? cartTotalPrice < config.CONFIG_order_min_price ? <Button disabled variant="button" onClick={handleMakeOrder} className="btn--action">Оформить заказ</Button>
                            : <Button variant="button" onClick={handleMakeOrder} className="btn--action">Оформить заказ</Button>
                            : <Button variant="button" onClick={handleMakeOrder} className="btn--action">Оформить заказ</Button> }
                        </div>
                    </div>
                </div>
                    ...
                    <div className="cart--empty">
                        ...
                        <h4>Ой, пусто!</h4>
                        <p className="minicart--empty-text">Добавьте товары в корзину.</p>
