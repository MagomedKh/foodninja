...
import {addProductToCart, decreaseProductInCart, removeProductFromCart, addBonusProductToCart} from '../../redux/actions/cart';
import Button from '@material-ui/core/Button';
import CloseIcon from '@mui/icons-material/Close';
import '../../css/cart.css';
import soon from '../../img/photo-soon.svg';

export default function MiniCartBonusProduct({productCart, productCount, productTotalPrice}) {
        ...
        <div className="minicart--product" data-product_id={productCart.id}>
            <div className="minicart--product-info">
                ...
                <div className="minicart--product-inner">
                    <div className="minicart--product-name">{productCart.title}</div>
                    <div className="minicart--product-info">
                    ...
                    <a className="minicart--product-remove" onClick={() => handleChooseBonusProduct({})} data-product_id={productCart.id}>	
                        <CloseIcon />
                    </a>
                </div>
            </div>
            <div className="minicart--product-result">
                <span><strike>{parseInt(productTotalPrice).toLocaleString('ru-RU')} &#8381;</strike> <span className="main-color">Подарок</span></span>

                <div className="minicart--product-quantity">
                    <Button className="btn--default product-decrease" onClick={() => handleChooseBonusProduct({})}>-</Button>
                    <input className="quantity" type="text" readOnly value={productCount} data-product_id={productCart.id} />
                    <Button className="btn--default product-add" disabled>+</Button>
                </div>
            </div>
        </div> 
    );
