...
export default function ProductModal() {
    ...
    useEffect(() => {
        if (productModal && productModal.categories?.length && categories) {
            let isDisabledCategoryFind = false;

            productModal.categories?.forEach((productCategoryId) => {
                const category = categories.find(
                    (category) => category.term_id === productCategoryId
                );
                if (category && _isCategoryDisabled(category).disabled) {
                    isDisabledCategoryFind = true;
                    if (category.limit_type === "block") {
                        setDisabled(true);
                    } else {
                        handleClose();
                    }
            ...
            if (!isDisabledCategoryFind && disabled) {
                setDisabled(false);
            }
        }
