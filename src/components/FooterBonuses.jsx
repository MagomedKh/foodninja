...
export default function BonusesProductsModal() {
  ...
  const {bonuses_items, cartTotalPrice, userCartBonusProduct, openBonusesProductsModal} = useSelector( ({products, cart, bonusesProductsModal}) => {
    return {
      bonuses_items: products.bonuses_items,
      userCartBonusProduct: cart.bonusProduct,
      cartTotalPrice: cart.totalPrice,
      openBonusesProductsModal: bonusesProductsModal.openBonusesProductsModal
    }
  });
    ...
    <div>
      <div className="footer-bonuses">
        <Container className="footer-bonuses__container">
            <div className="footer-bonuses__info" onClick={handleBonusesHandler}>
                Выбери подарок
            </div>
            <div className="footer-bonuses__points">
                <div 
                    className="footer-bonuses__points-bg"
                    style={{width: `${cartTotalPrice ? cartTotalPrice / maxBonusesPrice * 100 : 0}%`}}
                ></div>
                {bonuses_items.map(item => 
                    <div 
                        key={item.id}
                        className="footer-bonuses__point"
                        style={{left: `${item.limit / maxBonusesPrice * 100}%`}}
                    >
                        <div className="footer-bonuses__point-block">
                            {item.limit}
                            <span className="footer-bonuses__point-valute">₽</span>
                        </div>
                    </div>
                )}
            </div>
            ...
            <Grid  container spacing={4}>
              {bonusesItemsLocal &&
                bonusesItemsLocal.map((item, index) => 
                  <Grid key={item.id} item sm={12} md={4} sx={{ width: 1 }} >
                  <div 
                    className={`bonuses-modal__cart product product-item ${userCartBonusProduct.id === item.id ? "bonuses-modal__cart-active" : ""}`}
                  ...
                  >
                    ...
                    <div className="bonuses-modal__cart-price"><strike>{item.options._price} ₽</strike> <span className="main-color">Бесплатно</span></div>
                    <div className="bonuses-modal__cart-title">{item.title}</div>
                    <div className={`bonuses-modal__cart-content ${item.moreContent ? "bonuses-modal__cart-content--mod" : ""}`} dangerouslySetInnerHTML={{__html: item.content}}>

                    </div>
                    <div className="bonuses-modal__cart-info">
                      <div className="bonuses-modal__cart-cost">от {item.limit} ₽</div>
                      <div className="bonuses-modal__detail-wrapper">
                        {!item.moreContent && 
                          <div 
                            className="bonuses-modal__cart-detail"
                            onClick={() => setBonusesContentActive(item)}
                          >Подробнее</div>
                        }
                        {item.moreContent &&
                          <div 
                            className="bonuses-modal__cart-detail"
                            onClick={() => setBonusesContentNoActive(item)}
                          >Свернуть</div>
                        }
                      </div>
                    </div>
                      ...
                      { cartTotalPrice >= item.limit && userCartBonusProduct.id !== item.id &&
                  
                        <Button 
                          variant="button"
                          className="btn btn--action"
                          sx={{width: 1, mt: 2}}
                          onClick={() => handleChooseBonusProduct(item)}
                        >
                            Выбрать
                        </Button>
                      }
                      ...
                      {cartTotalPrice < item.limit &&
                        <Button 
                          ...
                          Не хватает {item.limit - cartTotalPrice} ₽
                        </Button>
                      }
