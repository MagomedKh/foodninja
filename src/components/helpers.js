   ...
   if (Object.keys(promocode).length) {
      ...
      if (typeDelivery) {
         ...
         if (promocode.typeDelivery === "self" && typeDelivery !== "self") {
            status = "error";
            alert = "Промокод отменен, т.к. действует только при самовывозе.";
            errors.push({
               code: "selfDelivery",
      ...
      if (
         ...
         if (
            ...
            if (notInCategory) {
               ...
               status = "error";
               alert = "Промокод отменен, т.к. действует с другими товарами.";
            }
         }
      }
      ...
      if (
         ...
         if (hasSale || productWithSale) {
            status = "error";
            alert = "Промокод отменен, т.к. не действует с товарами по скидке.";
            errors.push({
               code: "hasSale",
      ...
      const cartWithoutPromocodeProduct =
         promocode.type === "fixed_product" && promocodeProductInCart
            ? cartTotal - parseInt(promocode.promocodeProducts.options._price)
            : cartTotal;
...
export const _isCategoryDisabled = (category) => {
   ...
   if (category.timeLimitStart && category.timeLimitEnd) {
      ...
      const timeLimitEnd = set(new Date(), {
         ...
         minutes: isTimeAfterMidnight ? 59 : category.timeLimitEnd.slice(3, 5),
         seconds: 0,
      });
...
export const _checkAutoDiscount = ({
   ...
   // Проверка типа доставки
   if (typeDelivery) {
      if (discount.typeDelivery === "delivery" && typeDelivery !== "delivery") {
         return false;
      }
