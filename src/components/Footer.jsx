...
export default function Footer() {
  ...
  const {config, topMenu, currentPage} = useSelector( ({config, pages, products}) => {
    return {
      config: config.data,
			...
			<Container className="footer--container">
				{ currentPage === '/' && (
					<div className="about">
						...
						<div dangerouslySetInnerHTML={{__html: config.CONFIG_footer_text}}></div>
						<hr />
					</div>
				) }
				...
				<Grid container spacing={5}>
					...
					<Grid item sm={12} md={4} sx={{ width: 1 }}>
						...
						{ topMenu && (
							...
							{ topMenu.map( (item, index) => <li key={item.id}>
								{ item.target === '_blank' ? (
									<a href={item.url} target="_blank" rel="noreferrer" title={item.title}>{item.title}</a>
								) : (
									<Link onClick={() => handleClickPage(item)} to={item.url}>{item.title}</Link>
								) }
								</li> ) }
							</ul>
						) }
					...
					<Grid item sm={12} md={4} sx={{ width: 1 }} className="right-col">
						<h4>
							<a href={`tel:${config.CONFIG_format_phone}`}>{config.CONFIG_format_phone}</a>
						</h4>
				...
				<div className="footer-copyright">
					<Grid container spacing={5}>			
						<Grid item sm={12} md={8} sx={{ width: 1 }} className={ config.CONFIG_active_no_whitelabel && `right-col`}>
							{new Date().getFullYear()} © Все права защищены. Служба доставки еды <a href="/" title={config.siteTitle}>{config.CONFIG_company_name}</a>
						</Grid>
						{ !config.CONFIG_active_no_whitelabel &&
						<Grid item sm={12} md={4} sx={{ width: 1 }} className="right-col">
							<div className="creators">
								Платформа FoodNinja от <a href="https://iqmd.ru" target="_blank" rel="noreferrer">IQ Media</a>
							</div>
						</Grid>	}
